  ...
  _.extend(etch, {
    ...
    editableInit: function(e) {
      ...
      $('body').bind('mousedown.editor', function(e) {
        ...
        var t = $(target).closest('.editable');
        if (!t.length && $(target).not('.editable, .etch-editor-panel, .etch-editor-panel *, .etch-image-tools, .etch-image-tools *').size()) {
          // remove editor
          $editor.remove();
          ...
          $(document).trigger('etch.closed', $editable, $editor);
        }
      });
